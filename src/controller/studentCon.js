...
module.exports.createStudent = async function (req, res) {
  try {
    ...
    await systemLogMod.create(setFields)
    res
      .status(200)
      .send({
        status: true,
...
module.exports.getStudents = async function (req, res) {
  try {
    ...
    const data = await studentModel.find(filter).select(notShowed);
    // console.log(data)
    if (data.length == 0)
      return res
        .status(404)
...
await systemLogMod.create(fields)
...
module.exports.deleteStudent = async function (req, res) {
  try {
   ...
   let findStudent=await studentModel.find({$and:[delQueryData,{isDeleted:false}]})
...
let updatedStudent=await studentModel.findOneAndUpdate({$and:[delQueryData,{isDeleted:false}]},{$set:{isDeleted:true, deletedAt:date}},{new:true})


let fields={StudentId: updatedStudent._id, Event: "Deleted", TimeStamps:date, Description:"Student has been deleted" }
  ...
  await systemLogMod.create(fields)
    ...
    return res
      ...
      .send({
        ...
        message: "student successfully deleted", data:updatedStudent
      });
  } catch (error) {
